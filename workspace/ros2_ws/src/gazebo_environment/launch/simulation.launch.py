...
def generate_launch_description():
    ...
    # Set Gazebo resource paths for meshes and sdf
    install_dir_path = get_package_prefix(package_description) + "/share"
    robot_meshes_path = os.path.join(package_directory, "meshes")
    gazebo_resource_paths = [install_dir_path, robot_meshes_path]
    # Set environment variable for Ignition Gazebo resource path
    gazebo_resource_path_env = ":".join(gazebo_resource_paths)
    set_gz_resource_path = SetEnvironmentVariable(name="IGN_GAZEBO_RESOURCE_PATH", value=gazebo_resource_path_env)

    # World file and its launch config
    world = "demo_world"
    world_file = f"{world}.sdf"
    world_file_path = os.path.join(package_directory, "worlds", world_file)
    world_config = LaunchConfiguration("world")
    ...
    # Robot description from your URDF (use xacro command)
    urdf_file = "robot.urdf"
    robot_desc_path = os.path.join(package_directory, "urdf", urdf_file)
    robot_state_publisher_node = Node(
        package="robot_state_publisher",
    ...
    # Spawn arguments for initial pose
    declare_spawn_x = DeclareLaunchArgument("x", default_value="0.0", description="Spawn X")
    declare_spawn_y = DeclareLaunchArgument("y", default_value="0.0", description="Spawn Y")
    declare_spawn_z = DeclareLaunchArgument("z", default_value="0.5", description="Spawn Z")
    declare_spawn_R = DeclareLaunchArgument("R", default_value="0.0", description="Spawn Roll")
    declare_spawn_P = DeclareLaunchArgument("P", default_value="0.0", description="Spawn Pitch")
    declare_spawn_Y = DeclareLaunchArgument("Y", default_value="0.0", description="Spawn Yaw")

    # Spawn the robot in Gazebo from robot_description topic
    gz_spawn_entity = Node(
        package="ros_gz_sim",
        executable="create",
        name="my_robot_spawn",
        arguments=[
            "-name", "my_robot",
            "-allow_renaming", "true",
            "-topic", "robot_description",  # This topic is automatically published by robot_state_publisher
    ...
    # Ignition bridge topics for your robot's sensors and states
    ign_bridge = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
        name="ign_bridge",
        arguments=[
            ...
            "/tf@tf2_msgs/msg/TFMessage[ignition.msgs.Pose_V",
            "/odom@nav_msgs/msg/Odometry[ignition.msgs.Odometry",
            f"/world/{world}/model/my_robot/joint_state@sensor_msgs/msg/JointState[ignition.msgs.Model",
            "/imu@sensor_msgs/msg/Imu[ignition.msgs.IMU",
            "/navsat@sensor_msgs/msg/NavSatFix[ignition.msgs.NavSat",
            # Add laser scan here if you have one
    ...
    # Static TF publishers to fix frames (adjust frames according to your robot's links)
    navsat_tf_node = Node(
        name="navsat_stf",
        package="tf2_ros",
        executable="static_transform_publisher",
    ...
    # Bridge the OakD camera sensor topics
    robot_name = "my_robot"
    oakd_camera_bridge = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
